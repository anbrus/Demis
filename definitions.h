...
struct _EmulatorData {      //Глобальные общие данные эмулятора
  ...
  INT64 Takts;  //Счётчик инструкций
  void *Memory;
...
struct _HostData {                 //Глобальные общие данные оболочки
  HWND  hHostWnd;
  DWORD TaktFreq;                  //Тактовая частота в герцах
  DWORD RomSize;
};
...
extern "C" struct _EmulatorData* PASCAL InitEmulator(struct _HostData *pHostData);
...
extern "C" DWORD PASCAL SetInstrCounter(DWORD Owner,DWORD Value);
extern "C" DWORD PASCAL KillInstrCounter(DWORD Owner);
