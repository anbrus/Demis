...
void CMainFrame::OnFileMruPrj(UINT nID)
{
	...
	CString ext(".prj");
	theApp.OpenDemisDocument(PrjPathName, ext);
}

void CMainFrame::CloseAllWindows() {
	POSITION Pos = ChildWndList.GetHeadPosition();
	std::vector<_ChildWndInfo*> windowsInfo;
	while (Pos) {
		struct _ChildWndInfo* cwi = reinterpret_cast<_ChildWndInfo*>(ChildWndList.GetNext(Pos));
		if (cwi->DocType == ".prj") continue;
		windowsInfo.push_back(cwi);
	}
	for (struct _ChildWndInfo* cwi : windowsInfo) {
		cwi->pChildWnd->CloseWindow();
		cwi->pChildWnd->DestroyWindow();
	}
}
