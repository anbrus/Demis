...
CDocument* CDemis2000App::OpenDemisDocument(CString &PathName, CString& Type)
{
  CString DocString;
  CDocument* pDoc=NULL;
...
CElement* CDemis2000App::CreateExtElement(CString LibGUID, CString ElementName,BOOL ArchMode)
{
  if(ElementName.GetLength()>0) {
    BOOL Found=FALSE;
	int n = 0;
    for(n=0; n<16; n++) {
      if(this->LibGUID[n]==LibGUID) { Found=TRUE; break; }
    }
    if(!Found) return NULL;

    CElement *pElement=new CElement();
    if(!pElement->CreateInstance(ClsGUID[n])) {
      TRACE("CDemis2000App::CreateExtElement: pElement->CreateInstance failed");
      ASSERT(FALSE);
      return NULL;
    }
    if(!pElement->Create(ElementName,ArchMode)) {
      MessageBox(NULL,"Ошибка создания элемента","Ошибка",MB_OK|MB_ICONSTOP);
      delete pElement;
      return NULL;
    }
    return pElement;
...
BOOL CDemis2000App::EnumElemLibs(LONG Index, CString *pLibGUID, CString *pClsGUID)
{
  if(LibGUID[Index].GetLength()>0) {
    *pLibGUID=this->LibGUID[Index];
    *pClsGUID=this->ClsGUID[Index];
    return TRUE;
  }
  return FALSE;
}
...
void CDemis2000App::LoadLibraryInformation()
{
  ...
  do {
    ...
    Result=::RegEnumValue(hKey,Index,ValueName,&ValueNameSize,
      NULL,&Type,(LPBYTE)Value,&ValueSize);
    if(Result==ERROR_SUCCESS) {
      //wchar_t wcClsName[128];
      //LPOLESTR pmbClsID;
      //CLSID ClsID;
      //char mbClsID[64];

      //mbstowcs(wcClsName,ValueName,64);
      //CLSIDFromProgID(wcClsName,&ClsID);
      //StringFromCLSID(ClsID,&pmbClsID);
      //wcstombs(mbClsID,pmbClsID,64);
      //CoTaskMemFree(pmbClsID);

      LibGUID[Index]=ValueName;

      //mbstowcs(wcClsName,Value,64);
      //CLSIDFromProgID(wcClsName,&ClsID);
      //StringFromCLSID(ClsID,&pmbClsID);
      //wcstombs(mbClsID,pmbClsID,64);
      //CoTaskMemFree(pmbClsID);
...
void CALLBACK CDemis2000App::OnEmulatorFrozen(HWND hWnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime)
{
  ...
  MessageBox(NULL,"Емулятор \"висит\"","Ошибка",MB_OK);
...
void CDemis2000App::RunProg()
{
  ...
  ::SetThreadPriority(hRunThread,THREAD_PRIORITY_BELOW_NORMAL);
}
