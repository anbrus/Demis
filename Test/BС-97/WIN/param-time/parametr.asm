...
Vichitanie proc near       ; Процедура проверяет
	cmp ZnachDec1,0FFH ; была линажата соответствующая
...
ProsmotrInc proc near      ; Процедура проверяет
	...
	jmp konpinc	   ; инкрементирование номера точки
pri:	inc NumTochki
	cmp NumTochki,100
	jne pri1
	lea si,Tochki-2	
...
ProsmotrDec proc near      ; Процедура проверяет была
	...
	je prd             ; клавиша, если да, то производится 
	jmp konpdec        ; декрементирование номера точки
prd:	cmp NumTochki,0
	jne prd1
	lea si,Tochki+200
	mov NumTochki,100
...
ObrabotkaKl2 proc near     ; Процедура обрабатывает
	...
	mov MinMinKl,0     ; флаг после окончания
