...
CLedElement::CLedElement(BOOL ArchMode,CElemInterface* pInterface)
...
BOOL CLedElement::Reset(BOOL bEditMode,CURRENCY* pTickCounter,DWORD TaktFreq,DWORD FreePinLevel)
{
  HighLighted=FALSE;
...
CLedArchWnd::CLedArchWnd(CElement* pElement) : CElementWnd(pElement)
{
  Size.cx=5*2+6;
  Size.cy=5*2;
}
...
CLedConstrWnd::CLedConstrWnd(CElement* pElement) : CElementWnd(pElement)
{
  Size.cx=5*2;
  Size.cy=5*2;
}
...
void CLedElement::SetPinState(DWORD NewState)
{
  ...
  if(pArchElemWnd->IsWindowEnabled()) {
    ...
    ((CLedArchWnd*)pArchElemWnd)->DrawValue(&ArchDC);
  }
  if(pConstrElemWnd->IsWindowEnabled()) {
    CClientDC ConstrDC(pConstrElemWnd);
    ((CLedConstrWnd*)pConstrElemWnd)->DrawValue(&ConstrDC);
  }
}
