...
CButtonElement::CButtonElement(BOOL ArchMode,CElemInterface* pInterface)
...
BOOL CButtonElement::Reset(BOOL bEditMode,CURRENCY* pTickCounter,DWORD TaktFreq,DWORD FreePinLevel)
{
  Pressed=FALSE;
  ...
  if(!bEditMode && !NormalOpened) {
    pArchParentWnd->PostMessage(WMU_PINSTATECHANGED,CurState,(LPARAM)pInterface->hElement);
  }
...
CBtnArchWnd::CBtnArchWnd(CElement* pElement) : CElementWnd(pElement)
{
...
CBtnConstrWnd::CBtnConstrWnd(CElement* pElement) : CElementWnd(pElement)
{
...
void CButtonElement::OnLButtonDown()
{
  ...
  if(Drebezg) {
    DrebCounter=((DWORD)rand()*500)/RAND_MAX+500; //500..1000 нс
    DrebCounter=DrebCounter*TaktFreq/6/1000000;
    ...
    pArchParentWnd->SendMessage(WMU_PINSTATECHANGED,CurState,(LPARAM)pInterface->hElement);
  }
}
...
void CButtonElement::OnLButtonUp()
{
  if(!Fixable) {
    Pressed=FALSE;
    if(Drebezg) {
      DrebCounter=((DWORD)rand()*500)/RAND_MAX+500; //500..1000
      DrebCounter=DrebCounter*TaktFreq/6/1000000;
      pArchParentWnd->SendMessage(WMU_SET_INSTRCOUNTER,2,(LPARAM)pInterface->hElement);
    }else {
      CurState=(Pressed ? 1 : 0)^(NormalOpened ? 0 : 1)^FreePinLevel;
...
void CButtonElement::OnInstrCounterEvent()
{
  DrebCounter-=2;
  if(DrebCounter>0) {
    CurState=rand()&1;
