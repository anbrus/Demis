...
BOOL CStdElemApp::InitInstance()
{
	...
	ElementId[13].Icon.LoadMappedBitmap(IDB_CONNECTOR_ELEMENT);
...
CStdElemApp::CStdElemApp()
{
	...
	ElementId[13].Type = ET_ARCH;
	ElementId[13].Name = "Соединение";
	ElementId[13].ClsId = CurClsId;
}
...
std::shared_ptr<CElement> CStdElemLib::CreateElement(const CString& name, bool isArchMode, int id) {
	for (int n = 0; n < theApp.m_ElementsCount; n++) {
		...
		case 13: return std::make_shared <Connector>(isArchMode, id);
		}
	}
	return nullptr;
}
