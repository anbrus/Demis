...
CKbdElement::CKbdElement(BOOL ArchMode, int id)
...
CKbdArchWnd::CKbdArchWnd(CElementBase* pElement) : CElementWnd(pElement)
{
...
CKbdConstrWnd::CKbdConstrWnd(CElementBase* pElement) : CElementWnd(pElement)
{
...
BOOL CKbdElement::Reset(BOOL bEditMode, int64_t* pTickCounter, DWORD TaktFreq, DWORD FreePinLevel)
{
	memset(Pressed, FALSE, sizeof(Pressed));
	InputState = 0; OutputState = 0;
	ticksDrebezgEnd = 0;
...
void CKbdElement::RecalcOutputs()
{
	...
	for (int y = 0; y < KbdSize.cy; y++) {
		...
		for (int x = 0; x < KbdSize.cx; x++) {
			...
			if (ticksDrebezgEnd > *pTickCounter && (LastChangedKey.x == x) && (LastChangedKey.y == y)) {
				PressState = distributionBinary(rndEngine);
			}
			if (PressState) {
				if (FreePinLevel == 0) {
